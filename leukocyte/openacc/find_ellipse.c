...
MAT *GICOV(MAT * grad_x, MAT * grad_y) {
	...
	int MaxR = MAX_RAD + 2;
	host_gicov = (float *) malloc(grad_mem_size);

	int x, y;

	int n_circles = NCIRCLES;
	int n_points = NPOINTS;

#pragma acc kernels \
	copyin(host_grad_x[0:grad_m * grad_n], \
		host_grad_y[0:grad_m * grad_n], \
		host_tX[0:n_circles * n_points], \
		host_tY[0:n_circles * n_points], \
		host_sin_angle[0:n_points], \
		host_cos_angle[0:n_points]), \
	copy(host_gicov[0:grad_m * grad_n]), \
	private(i, j, k, n)
{
#pragma acc loop independent //gang(128) //(128), vector // gang(32), vector(32)
	for (i = MaxR; i < grad_n - MaxR; i++) {
#pragma acc loop independent //worker(128) //(64), vector //independent // worker(16), vector(32)
		for (j = MaxR; j < grad_m - MaxR; j++) {
			...
			for (k = 0; k < n_circles; k++) {
				...
				for (n = 0; n < n_points; n++) {
					...
					y = j + host_tY[(k * n_points) + n];
					x = i + host_tX[(k * n_points) + n];
					// Compute the combined gradient value at the current sample point
					int addr = x * grad_m + y;
					float p = host_grad_x[addr] * host_cos_angle[n] + host_grad_y[addr] * host_sin_angle[n];
				...
				mean = sum / ((float) n_points);
				float var = M2 / ((float) (n_points - 1));
...
MAT * structuring_element_f(int radius) {
...
#pragma acc kernels \
	copyin(host_gicov[0:max_gicov_m * max_gicov_n], host_strel[0:strel_m*strel_n]), \
	copy(host_img_dilated[0:max_gicov_m * max_gicov_n]), \
	private(i, j, el_i, el_j)
{
#pragma acc loop independent //gang(256) //independent
	for (i = 0; i < max_gicov_m; i++) {
#pragma acc loop independent //worker(128) //independent
		for (j = 0; j < max_gicov_n; j++) {
			...
			for (el_i = 0; el_i < strel_m; el_i++) {
				...
				if( (y >= 0) && (y < max_gicov_m) ) {
					for (el_j = 0; el_j < strel_n; el_j++) {
						...
						x = j - el_center_j + el_j;
						// Make sure we have not gone off the edge of the matrix
						if (x >= 0 && x < max_gicov_n && host_strel[(el_i * strel_n) + el_j] != 0) {
							// Determine if this is maximal value seen so far
			...
			// Store the maximum value found
			host_img_dilated[(i * max_gicov_n) + j] = max;
		}
	}
}
