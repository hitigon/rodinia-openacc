	...
	*/
	...
	for (frame_num = 1; frame_num <= Nf; frame_num++) {	 
		...
		for (cell_num = 0; cell_num < Nc; cell_num++) {	
			...
			//ellipseevolve(IMGVF, &(xci[cell_num]), &(yci[cell_num]), ri[cell_num], t, Np, (double) R, ycavg[cell_num]);
...
MAT **MGVF_f(MAT **IE, double vx, double vy, int Nc) {
	/*
	...
	*/
	...
	for (cell_num = 0; cell_num < Nc; cell_num++) {
		...
		for (i = 0; i < m; i++) {
			for (j = 0; j < n; j++) {
				m_set_val(IMGVF[cell_num], i, j, m_get_val(I, i, j));
			}
		}
		*/
	}
	...
	int total_mem_size = total_size * sizeof(float);

	int *host_I_offsets = (int *) malloc(sizeof(int) * Nc);
	int *host_m_array = (int *) malloc(sizeof(int) * Nc);
	int *host_n_array = (int *) malloc(sizeof(int) * Nc);

	float *host_I_all = (float *) malloc(total_mem_size);
	float *host_IMGVF_all = (float *) malloc(total_mem_size);

    float *host_mean_diff = (float *) malloc(sizeof(float) * Nc);

    for (i = 0; i < Nc; i++) {
        host_mean_diff[i] = 1.0;
    }
	...
	for (cell_num = 0; cell_num < Nc; cell_num++) {
		...
		for (i = 0; i < m; i++) {
			for (j = 0; j < n; j++) {
				...
				host_IMGVF_all[offset + (i * n) + j] = (float) m_get_val(I, i, j);
			}
		}
		offset += size;
	}
	...
	double one_over_e = 1.0 / epsilon;
#pragma acc kernels copyin(host_m_array[0:Nc], host_n_array[0:Nc], \
	host_I_offsets[0:Nc], host_I_all[0:total_size], host_mean_diff[0:Nc])\
    copy(host_IMGVF_all[0:total_size]), \
    private(cell_num, iter, i, j) 
{
...
#pragma acc loop independent
        ...
        for (iter = 0; iter < iterations; iter++) {
		    ...
		    I_offset = host_I_offsets[cell_num];
		    m = host_m_array[cell_num];
		    n = host_n_array[cell_num];
            float *imgvf_local = &(host_IMGVF_all[I_offset]);
            float *i_local = &(host_I_all[I_offset]);
		    double total_diff = 0.0;
		    // Compute the difference between each pixel and its eight neighbors
            
            if (host_mean_diff[cell_num] > converge) {
			for (i = 0; i < m; i++) {
				for (j = 0; j < n; j++) {
					int rowU = (i == 0) ? 0 : i - 1;
					...
					double old_val = host_IMGVF_all[I_offset + (i * n) + j];

					float U  = imgvf_local[(rowU * n) + j   ] - old_val;
					float D  = imgvf_local[(rowD * n) + j   ] - old_val;
					float L  = imgvf_local[(i    * n) + colL] - old_val;
					float R  = imgvf_local[(i    * n) + colR] - old_val;
					float UR = imgvf_local[(rowU * n) + colR] - old_val;
					float DR = imgvf_local[(rowD * n) + colR] - old_val;
					float UL = imgvf_local[(rowU * n) + colL] - old_val;
					float DL = imgvf_local[(rowD * n) + colL] - old_val;

					float UHe  = heaviside_f((U  *       -vy)  * one_over_e);
					float DHe  = heaviside_f((D  *        vy)  * one_over_e);
					float LHe  = heaviside_f((L  *  -vx     )  * one_over_e);
					float RHe  = heaviside_f((R  *   vx     )  * one_over_e);
					float URHe = heaviside_f((UR * ( vx - vy)) * one_over_e);
					float DRHe = heaviside_f((DR * ( vx + vy)) * one_over_e);
					float ULHe = heaviside_f((UL * (-vx - vy)) * one_over_e);
					float DLHe = heaviside_f((DL * (-vx + vy)) * one_over_e);
					...
					float vI = i_local[ (i * n) + j];
					double new_val = vHe - (one_over_lambda * vI * (vHe - vI));

					imgvf_local[(i * n) + j] = new_val;
            ...
            host_mean_diff[cell_num] = total_diff / (double) (m * n);
            } 
            /*iter++;*/
		}
	}
